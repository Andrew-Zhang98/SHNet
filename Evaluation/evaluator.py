...
class Eval_thread():
    def __init__(self, loader, method, dataset, output_dir, cuda, epoch_i=-1):
        self.loader = loader
        self.method = method
    ...
    def fast_run(self):
        ...
        self.LOG(
            '{} ({}): {:.4f} mae || {:.4f} S-measure || {:.4f} mean-fm ||.\n'
            .format(self.dataset, self.method, mae, s, mean_f))
        return Res
        # return '[cost:{:.4f}s] {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm ||.'.format(
        #     time.time() - start_time, self.dataset, self.method, mae, max_f, mean_f)
